...
select jsonb_build_object(
  'public_page_views', (
    select count(*) from public_events where event_name = 'page_viewed'
  ),
  'public_unique_browsers', (
    select count(distinct anonymous_id) from public_events where event_name = 'page_viewed'
  ),
  'public_browser_sessions', (
    select count(distinct session_id) from public_events where event_name = 'page_viewed'
  ),
  'public_active_browsers', (
    select count(distinct anonymous_id)
    from public_events
    where event_name = 'presence_heartbeat'
      and occurred_at >= now() - interval '75 seconds'
  ),
  'public_active_sessions', (
  ...
  'weekly_page_views', (
    select count(*) from public_events e cross join params p
    where e.event_name = 'page_viewed' and e.occurred_at >= p.week_start
  ),
  'weekly_visitors', (
    select count(distinct e.anonymous_id) from public_events e cross join params p
    where e.event_name = 'page_viewed' and e.occurred_at >= p.week_start
  ),
  'weekly_sessions', (
    select count(distinct e.session_id) from public_events e cross join params p
    where e.event_name = 'page_viewed' and e.occurred_at >= p.week_start
  ),
  'weekly_new_browsers', (
