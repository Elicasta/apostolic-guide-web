alter function analytics.dashboard_snapshot_v2() rename to dashboard_snapshot_v2_core;
...
with snapshot as (
  ...
  select
    value,
    (value #> '{v2,metrics}') || analytics.public_traffic_metrics_v2() as metrics
  from snapshot
)
select jsonb_set(value, '{v2,metrics}', metrics, true)
from merged;
$$;

revoke all on function analytics.dashboard_snapshot_v2_core() from public, anon, authenticated;
...
grant execute on function analytics.dashboard_snapshot_v2_core() to service_role;
