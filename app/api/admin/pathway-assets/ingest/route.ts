...
const actionSchema = z.discriminatedUnion("action", [prepareSchema, progressSchema, finalizeSchema, cancelSchema]);
...
function blobPath(pathwaySlug: string, studio: PathwayAssetIngestStudio, sessionId: string, fileName: string) {
  const now = new Date();
  return `pathway-assets/${pathwaySlug}/${studio}/${now.getUTCFullYear()}/${String(now.getUTCMonth() + 1).padStart(2, "0")}/${sessionId}/${sanitizePathwayAssetFilename(fileName)}`;
}
...
export async function GET(request: Request) {
  ...
  let query = service.from("studio_pathway_asset_uploads")
    .select("id,pathway_slug,studio,asset_type,file_name,mime_type,file_size,status,bytes_uploaded,error_message,expires_at,asset_id,storage_bucket,storage_path,created_at,updated_at")
    .eq("user_id", access.user.id)
    .in("status", ["prepared", "uploading", "uploaded", "failed"])
...
export async function POST(request: Request) {
  ...
  if (parsed.data.action === "prepare") {
    ...
    const path = blobPath(pathway.slug, studio, id, input.fileName);
    ...
    await audit(service, userId, "pathway_asset.ingest_prepare", id, {
      pathwaySlug: pathway.slug,
      studio,
      assetType,
      mediaKind,
      provider: PATHWAY_ASSET_STORAGE_PROVIDER,
      fileSize: input.fileSize
    });
    return NextResponse.json({ session: session.data, pathname: path, duplicateAsset });
  }
  ...
  if (parsed.data.action === "progress" || parsed.data.action === "fail") {
    ...
    const status = parsed.data.action === "fail" ? "failed" : "uploading";
  ...
  if (parsed.data.action === "cancel") {
    if (session.storage_bucket === PATHWAY_ASSET_STORAGE_PROVIDER && session.storage_path) {
      try { await del(session.storage_path); } catch (error) { console.warn("pathway blob cancel cleanup failed", error); }
    }
    const saved = await service.from("studio_pathway_asset_uploads").update({
      status: "cancelled",
      error_message: null,
  ...
  if (session.storage_bucket !== PATHWAY_ASSET_STORAGE_PROVIDER || !session.storage_path) {
    return NextResponse.json({ error: "This upload session is not backed by Vercel Blob." }, { status: 409 });
  }

  let blob;
  try {
    blob = await head(session.storage_path);
  } catch {
    return NextResponse.json({ error: "Upload has not finished reaching Vercel Blob yet. Retry finalization in a moment." }, { status: 409 });
  }
  if (Number(blob.size) !== Number(session.file_size)) {
    return NextResponse.json({ error: "Stored file size does not match the source file." }, { status: 409 });
  }
  ...
  const created = await service.from("studio_pathway_assets").insert({
    ...
    metadata: {
      ...
      blobEtag: blob.etag,
      ingestSessionId: session.id,
      lastModified: session.last_modified || undefined,
      ...mediaMetadata
    },
    created_by: userId,
    updated_by: userId,
  ...
  const finalized = await service.from("studio_pathway_asset_uploads").update({
    status: "finalized",
    bytes_uploaded: Number(session.file_size),
    asset_id: created.data.id,
    error_message: null,
    updated_at: now
  }).eq("id", session.id);
  if (finalized.error) return NextResponse.json({ error: finalized.error.message }, { status: 500 });

  await audit(service, userId, "pathway_asset.ingest_finalize", session.id, {
    assetId: created.data.id,
    pathwaySlug: session.pathway_slug,
    provider: PATHWAY_ASSET_STORAGE_PROVIDER,
    fileSize: Number(session.file_size),
    mediaKind
  });
  return NextResponse.json({ assetId: created.data.id, asset: created.data });
}
