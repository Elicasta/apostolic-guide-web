...
export async function POST(request: Request) {
  ...
  const styleResult = service ? await service.from("studio_visual_style_profile").select("instructions,metadata").eq("id", "apostolic-guide").maybeSingle() : null;
    ...
    body: JSON.stringify({
      ...
      input: [
        { role: "developer", content: [{ type: "input_text", text: [
          ...
          "Turn the user's creative request into one precise image-generation prompt that matches the saved Apostolic Guide visual language.",
  ...
  return NextResponse.json({ dataUrl: `data:image/webp;base64,${b64}`, prompt: imagePrompt, solModel, imageModel, size, revisedPrompt: result.data?.[0]?.revised_prompt ?? null });
}
