...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    cameraOffsetsMs: z.record(z.string(), z.number().finite()).optional(),
    externalAudioOffsetMs: z.number().finite().nullable().optional()
  }),
...
export async function PATCH(request: Request) {
  ...
  const result = await service.from("video_producer_projects")
    .select("id,status,parent_project_id,source_duration,transcript,edit_plan,director_metadata")
  ...
  if (parsed.data.action === "update-decisions") {
    if (!duration) return NextResponse.json({ error: "Source duration is required before camera decisions can be saved." }, { status: 409 });
    const normalized = normalizeVideoProducerCameraDecisions(parsed.data.decisions, duration, cameraIds);
  ...
  } else if (parsed.data.action === "manual-sync") {
    const allowedIds = new Set(cameraIds);
    const offsets = { ...multicam.analysis.cameraOffsetsMs };
    for (const [id, value] of Object.entries(parsed.data.cameraOffsetsMs || {})) if (allowedIds.has(id)) offsets[id] = value;
    multicam = {
      ...multicam,
      analysis: {
        ...multicam.analysis,
        status: "ready",
        error: null,
        cameraOffsetsMs: offsets,
        externalAudioOffsetMs: parsed.data.externalAudioOffsetMs === undefined ? multicam.analysis.externalAudioOffsetMs : parsed.data.externalAudioOffsetMs
      }
    };
  } else {
    if (parsed.data.sourceId === "external-audio") {
      locatorToDelete = multicam.externalAudio?.locator || null;
      multicam = {
        ...multicam,
        externalAudio: null,
        analysis: { ...multicam.analysis, externalAudioOffsetMs: null, externalAudioConfidence: null, externalAudioDuration: null, waveforms: { ...multicam.analysis.waveforms, "external-audio": [] } }
      };
    } else {
      const target = multicam.cameras.find((camera) => camera.id === parsed.data.sourceId);
      if (!target) return NextResponse.json({ error: "Camera source was not found." }, { status: 404 });
      locatorToDelete = target.locator;
      const { [parsed.data.sourceId]: _offset, ...cameraOffsetsMs } = multicam.analysis.cameraOffsetsMs;
      const { [parsed.data.sourceId]: _confidence, ...cameraConfidence } = multicam.analysis.cameraConfidence;
      const { [parsed.data.sourceId]: _duration, ...cameraDurations } = multicam.analysis.cameraDurations;
      const { [parsed.data.sourceId]: _waveform, ...waveforms } = multicam.analysis.waveforms;
      multicam = {
        ...multicam,
        cameras: multicam.cameras.filter((camera) => camera.id !== parsed.data.sourceId),
        analysis: { ...multicam.analysis, cameraOffsetsMs, cameraConfidence, cameraDurations, waveforms },
        editDecisions: multicam.editDecisions.map((decision) => decision.sourceId === parsed.data.sourceId ? { ...decision, sourceId: VIDEO_PRODUCER_PRIMARY_CAMERA_ID } : decision)
      };
  ...
  const update = await service.from("video_producer_projects").update({
    status,
    director_metadata: withVideoProducerMulticamMetadata(project.director_metadata, multicam),
    approval_fingerprint: null,
    approved_at: null,
    updated_by: access.user.id
  }).eq("id", project.id).select("director_metadata,status").single();
