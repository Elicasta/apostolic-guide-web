...
const schema = z.object({
  ...
  output: z.object({
    storageLocator: z.string().min(1).max(1000),
    ...
    assetIn: z.number().nonnegative(),
    assetOut: z.number().positive()
  }).optional()
});
...
export async function POST(request: Request) {
  ...
  const progress = { percent: parsed.data.percent ?? (parsed.data.status === "completed" || parsed.data.status === "failed" ? 100 : 0), stage: parsed.data.stage ?? parsed.data.status, heartbeatAt: new Date().toISOString() };
  ...
  try {
    ...
    if (job.provider_asset_id) {
      const existing = await service.from("video_producer_visual_assets")
        .select("id,storage_locator,duration")
    ...
    if (!assetId) {
      ...
      const asset = await service.from("video_producer_visual_assets").insert({
        ...
        provider_asset_id: job.provider_asset_id,
        source_url: job.source_url,
        creator: job.creator,
        license_name: job.license_name,
        license_url: job.license_url,
        license_snapshot: job.license_snapshot,
        retrieved_at: new Date().toISOString(),
        storage_provider: "vercel_blob",
        storage_locator: output.storageLocator,
        ...
        rights_flags: { thirdPartyRightsReviewRequired: true },
        metadata: { importedFromJobId: job.id, originalProviderMetadata: candidateMetadata },
        revision: 1,
    ...
    const sourceEnd = Math.max(sourceStart + 0.1, Number(metadata.sourceEnd || sourceStart + (output.assetOut - output.assetIn)));
    ...
    const placement = await service.from("video_producer_visual_placements").insert({
      ...
      source_end: Math.min(sourceEnd, sourceStart + Math.max(0.5, output.assetOut - output.assetIn)),
      asset_in: output.assetIn,
      asset_out: output.assetOut,
      fit: "cover",
      position_x: 0.5,
      position_y: 0.5,
      scale: 1,
