...
export async function POST(request: Request) {
  ...
  for (const format of [...new Set(parsed.data.formats)]) {
    ...
    const publishingAsset = await service.from("pathway_assets").insert({
      ...
      notes: parsed.data.replaceExisting
        ? `Regenerated by Video Studio from Pathway audio ${audioResult.data.content_hash}. The prior local render is retained until this replacement finishes successfully.`
        : `Generated by Video Studio from Pathway audio ${audioResult.data.content_hash}.`
    }).select("id").single();
