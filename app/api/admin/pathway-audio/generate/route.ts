...
export async function POST(request: Request) {
  ...
  const speech = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input: narration,
      response_format: "mp3",
      instructions: "Read as a calm, confident Bible study guide. Natural pacing, clear Scripture references, restrained emotion, no theatrical delivery."
    })
  });
  ...
  const upload = await service.storage.from("pathway-audio").upload(objectPath, audio, {
