...
export default async function AdminCreativeStudioPage({ searchParams }: { searchParams: Promise<{ project?: string; view?: string }> }) {
  ...
  return <section className="creative-hub-shell">
    <div className="creative-hub-switch" aria-label="Creative Studio views">
      <Link className={!libraryView ? "is-active" : ""} href="/admin/creative-studio"><Layers3 size={16}/><span><strong>Create / Edit</strong><small>Single · Carousel · Story</small></span></Link>
      <Link className={libraryView ? "is-active" : ""} href="/admin/creative-studio?view=library"><FolderOpen size={16}/><span><strong>Library</strong><small>Drafts · Ready · Published</small></span></Link>
    </div>
