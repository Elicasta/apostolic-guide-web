...
type EpisodeData = { episode: { id: string; title: string; episode_type: string; access_mode: string; status: string }; run: { id: string; name: string } | null; cues: CueRow[] };
type Pathway = { slug: string; title: string; summary: string; steps: Array<{ title: string; reference: string; explanation: string }> };

export default function EpisodeWorkspace({ data, pathway }: { data: EpisodeData; pathway: Pathway }) {
  const existing = new Set(data.cues.map((cue) => cue.label));
  return <main className="ag-studio ag-studio-workspace">
    <header className="ag-studio-topbar"><Link href="/studio" className="ag-studio-back"><ArrowLeft size={17}/> Studio</Link><span className="ag-studio-status">{data.episode.status}</span></header>
