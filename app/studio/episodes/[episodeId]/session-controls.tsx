...
export function SessionControls({ episodeId, runId }: { episodeId: string; runId: string }) {
  ...
  return <div className="ag-studio-session-controls">
    ...
    {outputUrl ? <div className="ag-studio-output-card"><div><span className="ag-studio-eyebrow">OBS Browser Source</span><strong>Program output ready</strong><code>{outputUrl}</code></div><div><button onClick={() => navigator.clipboard.writeText(outputUrl)}><Copy size={16}/> Copy</button><a href={outputUrl} target="_blank" rel="noreferrer"><ExternalLink size={16}/> Output</a><a href={consoleUrl}><Play size={16}/> Console</a></div></div> : null}
