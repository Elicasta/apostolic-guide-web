...
export default function StudioClient({ pathways }: { pathways: StudioPathway[] }) {
  ...
  return (
    <main className="ag-studio">
      <header className="ag-studio-topbar">
        <div className="ag-studio-brand"><span className="ag-studio-mark">AG</span><div><strong>Studio</strong><span>Production system</span></div></div>
        <span className="ag-studio-status">Pass 1 foundation</span>
      </header>

      <section className="ag-studio-hero">
        <div><span className="ag-studio-eyebrow">Apostolic Guide media</span><h1>Build the episode from the pathway.</h1><p>Plan solo episodes from real AG pathways, then carry the same system into live questions, polls, guests, and OBS.</p></div>
        <button className="ag-studio-primary"><Plus size={17} /> New episode</button>
      </section>

      <nav className="ag-studio-tabs">
        <button className={activeTab === "plan" ? "active" : ""} onClick={() => setActiveTab("plan")}><ListVideo size={16} /> Episode</button>
