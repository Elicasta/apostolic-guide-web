...
test("staging downscale happens after master-coordinate graphics are authored", () => {
  ...
  const applyProfile = worker.indexOf("apply_staged_inspection_profile(manifest)");
  const render = worker.indexOf("run_main_render_with_progress(payload, manifest, source_path, ass_path, main_path)");
