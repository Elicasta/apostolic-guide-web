...
export function InboxConversationControls({ conversationId, canReply, status }: { conversationId: string; canReply: boolean; status: string }) {
  ...
  async function send() {
    if (!body.trim()) return;
    if (await post({ action: "reply", body: body.trim() })) { setBody(""); setMessage("Sent."); window.setTimeout(() => window.location.reload(), 300); }
  }
  ...
  return <div className="inbox-controls">
    ...
    <div className="inbox-reply-box">
      {!canReply ? <div className="inbox-window-warning"><Clock3 size={15}/><span>Instagram's reply window is closed. This person needs to message again before a manual reply can be sent.</span></div> : null}
      <textarea value={body} onChange={(e) => setBody(e.target.value)} maxLength={10000} placeholder="Write a reply…" disabled={!canReply || busy}/>
      <div><small>{body.length}/10000</small><button type="button" className="button" onClick={send} disabled={!canReply || busy || !body.trim()}>{busy ? "Sending…" : <>Send reply <Send size={15}/></>}</button></div>
