...
import type { StudioMember } from "@/studio-roles";
import { STUDIO_ROLE_DESCRIPTIONS, STUDIO_ROLE_LABELS, type StudioRole } from "@/studio-permissions";
...
export function StudioTeamManager({ members, currentUserId, currentRole }: { members: StudioMember[]; currentUserId: string; currentRole: StudioRole }) {
  ...
  async function mutate(payload: unknown, busyKey: string) {
    ...
    setBusy("");
    if (!response.ok) {
      setMessage(result.error ?? "Could not update team access.");
      return false;
    }
    setMessage("Saved.");
  ...
  return <div className="team-layout">
    <section className="admin-card team-members-card">
      ...
      <div className="team-list">{members.map((member) => {
        ...
        const onlyOwner = self && member.role === "owner" && members.filter((item) => item.role === "owner").length === 1;
        return <div className="team-row" key={member.id}>
          ...
          <select aria-label={`Role for ${member.email}`} value={member.role} disabled={Boolean(busy) || ownerProtected || onlyOwner} onChange={(event) => mutate({ action: "role", memberId: member.id, role: event.target.value }, `role:${member.id}`)}>
