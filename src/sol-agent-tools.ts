...
export const SOL_AGENT_TOOLS = [
  {
    ...
    description: "Read the reconciled Apostolic Guide Manager state: all six specialist lanes, manager priorities, KPIs, content coverage, Forge production state, Creative Project production state, current execution, failures, and review gates. Use this before making claims about what needs attention.",
    strict: true,
  ...
  },
  {
    ...
    description: "Read Forge's deterministic production queue and Pathway-level audio, carousel, and YouTube state. Use this for questions about what Forge can make, what is already owned by a current job, and what production gap should move next.",
    strict: true,
    parameters: EMPTY_OBJECT
  },
  {
  ...
  },
  {
    type: "function",
    name: "scan_workspace",
    description: "Run the reconciled Sol manager cycle. It refreshes current evidence even when execution is paused, discovers proposals, suppresses duplicate review work, and refreshes all six specialist priorities. It never publishes content.",
    strict: true,
    parameters: EMPTY_OBJECT
  },
  {
    type: "function",
    name: "list_proposals",
    description: "List only current pending Sol proposals after reconciliation, with IDs, risk, priority, pathways, and suggested constraints.",
    strict: true,
    parameters: EMPTY_OBJECT
  },
  {
...
export async function executeSolAgentTool(name: SolAgentToolName, rawArgs: unknown, context: ToolContext): Promise<SolAgentToolResult> {
  ...
  if (name === "scan_workspace") {
    ...
    return {
      ok: true,
      message: `Manager cycle complete. ${next.proposals.filter((item) => item.status === "pending").length} current proposals are waiting; Forge sees ${forge.summary.total} production task${forge.summary.total === 1 ? "" : "s"}.`,
      data: {
        ...currentStatus(next),
