...
type EventPayload = { name: EventName; path: string; anonymousId: string; properties?: EventProperties };
...
function getCampaignAttribution(): EventProperties {
  ...
  if (campaign) {
    const attribution: EventProperties = { _utm_source: search.get("utm_source"), _utm_medium: search.get("utm_medium"), _utm_campaign: campaign, _utm_content: search.get("utm_content"), _utm_term: search.get("utm_term") };
...
function setCrossDomainAnonymousCookie(id: string) {
  const isProductionDomain = location.hostname === "apostolicguide.com" || location.hostname.endsWith(".apostolicguide.com");
...
function sendEvent(payload: EventPayload) {
  const body = JSON.stringify({ ...payload, properties: { ...(payload.properties ?? {}), ...getCampaignAttribution() }, sessionId: getSessionId(), referrer: document.referrer || null, viewportWidth: window.innerWidth });
  if (navigator.sendBeacon) { navigator.sendBeacon("/api/analytics/events", new Blob([body], { type: "application/json" })); return; }
  void fetch("/api/analytics/events", { method: "POST", body, headers: { "content-type": "application/json" }, keepalive: true });
}
export function trackEvent(name: EventName, properties?: EventProperties) {
  ...
  sendEvent({ name, path: `${location.pathname}${location.search}`, anonymousId, properties });
}
...
export function ProductAnalytics() {
  ...
  useEffect(() => {
    if (pathname.startsWith("/admin")) return;
    const heartbeat = () => { if (document.visibilityState === "visible") trackEvent("presence_heartbeat", { visible: true }); };
    heartbeat();
  ...
  useEffect(() => {
    ...
    const updateScroll = () => {
      ...
      maximumScrollPercent = scrollable <= 0 ? 100 : Math.max(maximumScrollPercent, Math.min(100, Math.round((window.scrollY / scrollable) * 100)));
    };
    const updateVisibility = () => {
      if (document.visibilityState === "hidden" && activeStartedAt !== null) { activeMilliseconds += Date.now() - activeStartedAt; activeStartedAt = null; }
      else if (document.visibilityState === "visible" && activeStartedAt === null) activeStartedAt = Date.now();
    };
    ...
    const maybeComplete = () => {
      ...
      sent = true;
      trackEvent("article_completed", { contentKey: pathname.slice("/articles/".length), activeSeconds: currentActiveSeconds(), maximumScrollPercent });
    };
    updateScroll();
    window.addEventListener("scroll", updateScroll, { passive: true });
    document.addEventListener("visibilitychange", updateVisibility);
    const interval = window.setInterval(maybeComplete, 5000);
    return () => { window.removeEventListener("scroll", updateScroll); document.removeEventListener("visibilitychange", updateVisibility); window.clearInterval(interval); };
  }, [pathname]);
  ...
  useEffect(() => {
    const handleClick = (event: MouseEvent) => {
      ...
      if (url.hostname === "app.apostolicguide.com") {
        trackEvent("app_link_clicked", { target: `${url.pathname}${url.search}`, placement: url.searchParams.get("placement"), origin: url.searchParams.get("origin") });
        return;
      }
      if (location.pathname === "/search" && anchor.classList.contains("search-result")) {
        trackEvent("search_result_opened", { query: new URLSearchParams(location.search).get("q"), destination: `${url.pathname}${url.search}` });
      }
    };
