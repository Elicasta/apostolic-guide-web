...
const items: BottomItem[] = [
  { href: "/admin", label: "Home", icon: House, permission: "view_workspace", active: (pathname) => pathname === "/admin" },
  { href: "/admin/carousel-studio", label: "Create", icon: Layers3, permission: "manage_content", active: (pathname) => pathname.startsWith("/admin/carousel-studio") },
...
export function StudioStandaloneBottomNav({ role }: { role: StudioRole }) {
  ...
  return <nav className="studio-standalone-bottom-nav" aria-label="Studio app navigation">
    {visible.map((item) => {
