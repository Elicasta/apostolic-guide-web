...
export async function recoverStaleSolRuns() {
  ...
  const result = await service.from("sol_operator_runs")
    .select("id,proposal_id,recipe_key,status,attempt_count,max_attempts,lease_expires_at,heartbeat_at,created_at,updated_at")
  ...
  for (const row of result.data ?? []) {
    const queuedSince = timestamp(row.created_at) ?? now;
    ...
    if (automatic) {
      ...
      await service.from("sol_operator_events").insert({ run_id: row.id, proposal_id: row.proposal_id ?? null, event_type: "run.recovered", detail: { reason: staleRunning ? "lease_expired" : "queue_orphaned", next_retry_at: nextRetryAt, attempt_count: attemptCount } });
      recovered += 1;
      continue;
    }
    ...
    if (updated.error) throw updated.error;
    await service.from("sol_operator_events").insert({ run_id: row.id, proposal_id: row.proposal_id ?? null, event_type: "run.stalled", detail: { reason: staleRunning ? "lease_expired" : "queue_orphaned", attempt_count: attemptCount, requires_user_context: !canSolRunRecoverWithoutUser(row.recipe_key) } });
    stalled += 1;
  }
...
export async function listRunnableSolRunIds(limit = 3, automaticOnly = false) {
  ...
  const now = Date.now();
  return (result.data ?? [])
    .filter((row) => (!automaticOnly || canSolRunRecoverWithoutUser(row.recipe_key)) && (row.status === "queued" || !row.next_retry_at || (timestamp(row.next_retry_at) ?? Infinity) <= now))
