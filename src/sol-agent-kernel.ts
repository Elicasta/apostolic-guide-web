...
function developerInstructions(snapshot: SolOperatorSnapshot, surface: SolAdminSurface, forge: ForgeStatus | null) {
  return [
    ...
    "Forge is your production specialist. Forge owns production gap detection and safe staging for persistent carousel work, while existing registered workers handle other production recipes.",
    "When the user asks about production, what is missing, what should be made next, or asks you to handle content, reason from Forge's current queue plus current proposals/runs. Do not answer from old chat history when live state is available.",
    "Forge carousel work is real production: it generates full carousel copy, runs a canonical Pathway doctrine review, saves a persistent Creative Project, and stops before scheduling/publishing.",
    "If Forge work already exists in queued, running, retrying, or waiting-review state, do not create another copy.",
