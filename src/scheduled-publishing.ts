...
async function sourceForInstagram(service: NonNullable<ReturnType<typeof createServiceClient>>, slug: string, metadata: PublicationMetadata) {
  if (metadata.source_kind === "clip" && metadata.clip_id) {
    const clipResult = await service.from("pathway_social_clips")
      .select("id,asset_id,status,output_url,caption")
      .eq("id", metadata.clip_id)
...
async function publishYouTube(publicationId: string, slug: string, metadata: PublicationMetadata) {
  ...
  const body = {
    snippet: { title: kit.youtubeTitle, description, tags: kit.youtubeTags, categoryId: "27", defaultLanguage: "en", defaultAudioLanguage: "en" },
  ...
  const updates = await Promise.all([
    service.from("pathway_publications").update({
      status: "published", external_post_id: result.id, published_url: publishedUrl, published_at: now, error_message: null,
      metadata: { ...metadata, actual_privacy: result.status?.privacyStatus ?? null, title: kit.youtubeTitle }
    }).eq("id", publicationId),
...
async function publishInstagram(publicationId: string, slug: string, metadata: PublicationMetadata) {
  ...
  const tags = kit.socialHashtags.map((tag) => tag.startsWith("#") ? tag : `#${tag.replace(/^#+/, "")}`).join(" ");
  ...
  const updates = await Promise.all([
    service.from("pathway_publications").update({
      status: "published", external_post_id: published.id, published_url: permalink, published_at: now, error_message: null,
      metadata: { ...metadata, container_id: container.id, caption, source_kind: source.sourceKind, source_id: source.sourceId }
    }).eq("id", publicationId),
...
export async function executeScheduledPublication(publicationId: string) {
  const service = createServiceClient();
  if (!service) throw new Error("Supabase service access is not configured.");
  const claimed = await service.from("pathway_publications")
    .update({ status: "publishing", error_message: null, updated_at: new Date().toISOString() })
    .eq("id", publicationId)
    .eq("status", "scheduled")
    .select("id,pathway_slug,platform,metadata")
    .maybeSingle();
