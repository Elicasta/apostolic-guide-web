...
import { CalendarDays, CheckCircle2, Film, Image as ImageIcon, Instagram, Loader2, Save, Send, Sparkles, UploadCloud, Youtube } from "lucide-react";
...
export function CustomMediaPublishingClient() {
  ...
  function payload(action: "generate" | "save" | "publish") {
    return {
      action,
  ...
  async function generate() {
    ...
    try {
      const data = await jsonRequest<{ generated: Record<string, unknown> }>("/api/admin/publishing/custom-media", { method: "POST", body: JSON.stringify(payload("generate")) });
      const generated = data.generated || {};
      patch({
        title: typeof generated.title === "string" ? generated.title : form.title,
        description: typeof generated.description === "string" ? generated.description : form.description,
        caption: typeof generated.caption === "string" ? generated.caption : form.caption,
        altText: typeof generated.altText === "string" ? generated.altText : form.altText,
        hashtags: join(generated.hashtags),
        tags: join(generated.tags),
        internalTags: join(generated.internalTags)
      });
      setMessage("Sol draft ready. Edit anything you want before saving or publishing.");
    } catch (reason) {
  ...
  return <section className="creative-publishing-shell creative-guided-publishing">
    ...
    <section className="admin-card">
      <div className="calendar-section-head"><div><span className="section-kicker">1 · Source</span><h2>Upload finished media</h2><p>Images and videos upload directly to the existing private Vercel Blob asset store. YouTube is enabled only for video.</p></div>{isVideo ? <Film size={25}/> : <ImageIcon size={25}/>}</div>
      ...
      <div className="creative-publish-actions">
        ...
        {form.assetId ? <span><CheckCircle2 size={15}/> Asset registered</span> : null}
      </div>
      {previewUrl ? <div className="creative-publish-preview">{isVideo ? <video src={previewUrl} controls preload="metadata"/> : <img src={previewUrl} alt={form.altText || form.title || "Custom media preview"}/>}</div> : null}
      <p><strong>{selectedPathway?.title}</strong> · {selectedPathway?.summary}</p>
    </section>

    {form.assetId ? <section className="admin-card">
      <div className="calendar-section-head"><div><span className="section-kicker">2 · Package</span><h2>Tell Sol what this post is about</h2><p>The brief guides the copy. The selected Pathway supplies the doctrinal lane and Scripture context. Generated fields stay editable.</p></div><Sparkles size={25}/></div>
      <label><span>Brief for Sol</span><textarea rows={4} value={form.brief} onChange={(event) => patch({ brief: event.target.value })} placeholder="Example: This graphic is about Jesus Is God. Make the caption direct and point people into the pathway."/></label>
      <div className="creative-publish-actions"><button type="button" className="button primary" onClick={() => void generate()} disabled={Boolean(working)}>{working === "sol" ? <Loader2 className="spin" size={15}/> : <Sparkles size={15}/>} Generate with Sol</button><span>Manual editing always remains available.</span></div>
      ...
      <label><span>{form.platform === "youtube" ? "YouTube description" : "Description / internal summary"}</span><textarea rows={5} value={form.description} onChange={(event) => patch({ description: event.target.value })}/></label>
      <label><span>Instagram caption</span><textarea rows={6} value={form.caption} onChange={(event) => patch({ caption: event.target.value })} disabled={form.platform === "youtube"}/></label>
      <div className="admin-form-grid">
      ...
      <div className="creative-publish-actions"><button type="button" className="button" onClick={() => void saveDraft()} disabled={Boolean(working)}>{working === "save" ? <Loader2 className="spin" size={15}/> : <Save size={15}/>} Save draft + calendar</button></div>
    </section> : null}
    ...
    {form.assetId ? <section className="admin-card">
      ...
      <div className="creative-publish-actions"><button type="button" className="button primary" onClick={() => void publish()} disabled={!canPublish || Boolean(working)}>{working === "publish" ? <Loader2 className="spin" size={15}/> : mode === "schedule" ? <CalendarDays size={15}/> : <Send size={15}/>} {mode === "schedule" ? "Add to publishing calendar" : `Publish to ${form.platform === "instagram" ? "Instagram" : "YouTube"}`}</button></div>
    </section> : null}

    {error ? <p className="admin-error">{error}</p> : null}<p className="content-calendar-status">{message}</p>

    {recent.length ? <section className="admin-card"><div className="calendar-section-head"><div><span className="section-kicker">Recent custom media</span><h2>Resume a saved upload</h2><p>Custom media remains organized in Pathway Assets instead of becoming a separate orphan library.</p></div></div><div className="calendar-ready-list">{recent.slice(0, 8).map((asset) => <article key={asset.id}><div className="calendar-ready-type"><i>{pathwayAssetMediaKind(metadataString(asset.metadata, "mimeType") || metadataString(asset.metadata, "mime")) === "video" ? "VD" : "IM"}</i><div><span>{asset.status} · {asset.pathway_slug.replaceAll("-", " ")}</span><strong>{asset.title}</strong></div></div><button type="button" className="button" onClick={() => loadAsset(asset)}>Open</button></article>)}</div></section> : null}
  </section>;
}
