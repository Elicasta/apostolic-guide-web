...
7. An approved reply is scheduled with a lane-specific human delay.
8. Keyword requests receive a public acknowledgement and the existing private `OPEN` handshake. Other matched Pathways use the same private handoff.

Repeated contention from one person on one post receives no second doctrinal reply. Hostile, sensitive, spam, ambiguous, and failed comments receive no automatic fallback.
