...
def package_with_bumpers(manifest, main_file, final_file, directory):
    """Build short bumpers, then concatenate without re-encoding the entire episode."""
    plan = manifest["renderPlan"]
    parts = []
    if plan.get("intro"):
    ...
    # All pieces are generated by this worker with matching H.264/AAC geometry. Stream
    # copy avoids a second full-episode x264 pass and keeps packaging nearly constant-time.
    subprocess.run([
        "ffmpeg", "-hide_banner", "-loglevel", "warning", "-y",
        "-f", "concat", "-safe", "0", "-i", concat_file,
        "-c", "copy", "-movflags", "+faststart", final_file
    ], check=True)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ag-video-producer-render-") as directory:
        ...
        base.callback(payload, "rendering", 89, "Building branded intro and outro")
