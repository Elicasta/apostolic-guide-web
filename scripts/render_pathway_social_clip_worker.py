...
def run_ffmpeg_with_progress(command: list[str], duration: float, payload: dict) -> None:
    ...
    for raw in process.stdout:
        ...
        if percent >= last_sent + 4:
            callback(payload, "rendering", progress=percent, stage="Rendering original style")
            last_sent = percent
    ...
    callback(payload, "rendering", progress=84, stage="Rendering original style")
...
def render(payload: dict, output: Path, cover: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        callback(payload, "rendering", progress=6, stage="Preparing original cut")
        ...
        callback(payload, "rendering", progress=10, stage="Rendering original style")
